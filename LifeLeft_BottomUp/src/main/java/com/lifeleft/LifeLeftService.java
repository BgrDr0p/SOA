...
import exceptions.LifeLeftErrors;
...
public class LifeLeftService {
    ...
    public String  anneeRestantesAVivre (String prenom, String sexe, Integer anneeNaissance) throws LifeLeftException {
        ...
        if(anneeNaissance> 2020){
            LifeLeftErrors L = new LifeLeftErrors();
            L.setError("1234");
            L.setMessageError("Date dans le futur !");

            throw  new LifeLeftException("Année invalide", L);
    ...
    @WebMethod()
    public int createClient(String login, String password){
        return ThreadLocalRandom.current().nextInt(100,900);
    }

    @WebMethod()
    public String commanderCompteARebourds(Integer clientID) {
        return "Votre commande pour le client " + clientID + " est validée";
    }
